...
inline BiosAttrMap JsonToBiosAttribute(const nlohmann::json& j)
{
    BiosAttrMap iMap;
                           ...
                           const std::string& systemName)
{
    ...
    BiosAttrMap patchMap =
        JsonToBiosAttribute(bJsonPatchObject["Attributes"]);
    if (patchMap.size() == 0)
    {
        asyncResp->res.jsonValue["message"] = "Not valid input";
        asyncResp->res.jsonValue["status"] = "error";
                          ...
                          const std::string& systemName)
{
    ...
    BiosAttrMap table = JsonToBiosAttribute(biosPostJsonObject["Attributes"]);
    // make dbus call transfer the data
