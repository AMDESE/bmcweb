...
class RDEServiceHandler : public std::enable_shared_from_this<RDEServiceHandler>
{
  public:
    ...
    ~RDEServiceHandler()
    {
        prepareAndSendCollectionResponse();
    ...
    inline void bootstrapProcess()
    {
        ...
        dbus::utility::getSubTreePaths(
            ...
            {
                BMCWEB_LOG_ERROR(
                    "updateDeviceProp: getSubTreePaths DBUS error: {}",
                    ec.message());
            ...
            for (const std::string& path : objPaths)
            {
                ...
                {
                    BMCWEB_LOG_ERROR(
                        "updateDeviceProp: Failed to find '/' in {}", path);
                    continue;
                }

                BMCWEB_LOG_DEBUG(
                    "updateDeviceProp: Found device with ObjectPath {}", path);
                ...
                sdbusplus::asio::getAllProperties(
                    *crow::connections::systemBus, std::string(pldmService),
                    ...
                    {
                        BMCWEB_LOG_ERROR(
                            "updateDeviceProp: DBUS response error: {}",
                            errCode.message());
                        return;
                    }
                    ...
                    {
                        BMCWEB_LOG_ERROR(
                            "updateDeviceProp: Failed to unpack properties");
                        return;
                    }
                    ...
                    {
                        BMCWEB_LOG_DEBUG(
                            "updateDeviceProp: UUID mismatch: expected");
                        return;
                    }
                    // UUID matched — update the active device UUID
                    self->deviceUUID = uuid;
                    BMCWEB_LOG_INFO(
                        "updateDeviceProp: UUID {} matched and updated", uuid);
                    ...
                    BMCWEB_LOG_INFO(
                        "updateDeviceProp: Successfully updated resourceData for UUID {}",
                        uuid);
                    self->process(self);
                });
            }
  ...
  private:
    /**
     * @brief Prepares and sends a schema-specific Redfish collection response.
     *
     * This method constructs the standard Redfish collection response for a
     * given schema (e.g., "Processors", "NetworkAdapters") by setting
     * fields such as
     ...
     * pattern.
     */
    inline void prepareAndSendCollectionResponse()
    {
        if (responseSent)
            return;
        ...
        asyncResp->res.jsonValue["Members@odata.count"] =
            static_cast<int>(asyncResp->res.jsonValue["Members"].size());
    ...
    inline void operationGet(const std::shared_ptr<RDEServiceHandler>& self)
    {
        ...
        const uint32_t operationId = self->nextOperationId();
        ...
        crow::connections::systemBus->async_method_call(
            ...
            const std::string matchRule =
                "type='signal',sender='" + std::string(pldmService) +
                "',"
                "interface='" +
                std::string(rdeSignalInterface) +
                "',"
                "member='" +
                std::string(rdeSignalMember) +
                "',"
                "path='" +
                static_cast<const std::string&>(objPath) + "'";
                ...
                if (payloadIt != changed.end())
                {
                    ...
                    try
                    {
                        self->asyncResp->res.jsonValue["Payload"] =
                            self->handleDeferredBindings(payloadStr);
            ...
            rdeOperationMethod, operationId, rdeOperationTypeRead, self->subUri,
            self->deviceUUID, self->deviceEID, rdeEmptyPayload,
            rdePayloadFormatInline, rdeEncodingFormatJSON, self->schema);
    }

    inline std::string handleDeferredBindings(const std::string& bejJsonInput)
    {
...
inline void requestRoutesRDEService(crow::App& app)
{
    ...
    constexpr const char* networkSchemaName = "NetworkAdapters";
    ...
    BMCWEB_ROUTE(
        ...
        handleRDEServicePath(app, req, asyncResp, systemId, deviceId,
                             vendorPath, procSchemaName, baseUri);
    });
    ...
    BMCWEB_ROUTE(
        ...
        handleRDEServiceRoot(app, req, asyncResp, systemId, deviceId,
                             procSchemaName, baseUri);
    });

    BMCWEB_ROUTE(
        app, "/redfish/v1/Systems/<str>/NetworkAdapterCollection/<str>/<path>")
        .privileges(redfish::privileges::patchProcessor)
        .methods(boost::beast::http::verb::head, boost::beast::http::verb::get,
                 boost::beast::http::verb::patch)(
            [networkSchemaName,
             &app](const crow::Request& req,
                   ...
                   const std::string& vendorPath) {
        std::string baseUri = "/redfish/v1/Systems/" + systemId +
                              "/NetworkAdapterCollection/" + deviceId;

        handleRDEServicePath(app, req, asyncResp, systemId, deviceId,
                             vendorPath, networkSchemaName, baseUri);
    });

    BMCWEB_ROUTE(app,
                 "/redfish/v1/Systems/<str>/NetworkAdapterCollection/<str>")
        .privileges(redfish::privileges::getProcessor)
        .methods(boost::beast::http::verb::get)(
            [networkSchemaName,
             &app](const crow::Request& req,
